...
export const documentMasterService = {
  ...
  create: async (body: { discom: string; title: string; canGenerate?: boolean; sortOrder?: number }): Promise<{ data: DocumentMaster }> => {
    const { data } = await api.post('/document-master', body);
    return data;
  },

  update: async (id: string, body: { title?: string; canGenerate?: boolean; sortOrder?: number; isActive?: boolean }): Promise<{ data: DocumentMaster }> => {
